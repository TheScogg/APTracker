...
- Frontend new/edit issue flows upload new `dataUrl` photos to Storage first and write `attachments` docs.
- Phase 4 PR4 cutover removes frontend writes to legacy `issue.photos[]`; `attachments` + `photoCount` are now the canonical photo write path.
...
- Phase 6 begins timeline read cutover: frontend hydrates `eventHistory` from `issues/{issueId}/events/*` and prefers it over `statusHistory` during rendering/export.
- Phase 7 hard cutover removes `statusHistory` fallback from primary render/export status timelines and recently-updated sorting logic.
